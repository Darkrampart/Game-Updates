...
Spike.prototype.type = "spike";
 ...
 function powerDown(pos) {
   this.basePos = this.pos = pos.plus(new Vector(0.4, 0.2));
   this.size = new Vector(0.4, 0.4);
   // Make it go back and forth in a sine wave.
   this.wobble = Math.random() * Math.PI * 3;
 }
 powerDown.prototype.type = "powerDown";
...
var wobbleSpeed = 8, wobbleDist = 0.07, powerUpDist = 0.15, powerUpSpeed = 15;
...
powerUp.prototype.act = function(step) {
  this.wobble += step * powerUpSpeed;
  var wobblePos = Math.sin(this.wobble) * powerUpDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

powerDown.prototype.act = function(step) {
  this.wobble += step * powerUpSpeed;
  var wobblePos = Math.sin(this.wobble) * powerUpDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    this.player.size = new Vector(.5,.5);
    this.status == null;
  }
    ...
    this.player.size = new Vector(.8, 1.5);
    this.status == null;
  }
